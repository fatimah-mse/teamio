...
export default function TaskOverviewChart() {
    ...
    const fetchTaskStatus = async () => {
        try {
            ...
            const response = await axios.get('https://task-manager-apis-t2dp.onrender.com/api/tasks/status', {
            ...
            const formatted: TaskStatus[] = Object.entries(raw)
                .map(([key, value]) => ({
            ...
            Swal.fire({
                title: 'Error!',
                text: err.response?.data?.message || 'Failed to load task statuses.',
                icon: 'error',
                ...
                <div className="my-8">
                    ...
                    {selectedStatus && (
                        <div className="mt-6 text-sm text-gray-600">
                            <p>
                                Showing <strong>{data.find(d => d.status === selectedStatus)?.count}</strong> tasks with status: <span className="text-indigo-600 font-semibold">{selectedStatus}</span>
                            </p>
                        </div>
                    )}
                </div>
